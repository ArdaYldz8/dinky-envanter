...
CREATE TABLE IF NOT EXISTS customer_transactions (
    ...
    transaction_type VARCHAR(50) NOT NULL, -- 'Satış', 'Alış', 'Tahsilat', 'Ödeme', 'Borç', 'Alacak'
...
BEGIN
    ...
    WITH ordered_transactions AS (
        SELECT
            ...
            ct.description,
            CASE
                WHEN ct.transaction_type IN ('Satış', 'Alacak', 'Tahsilat') THEN ct.amount
                WHEN ct.transaction_type IN ('Alış', 'Borç', 'Ödeme') THEN -ct.amount
                ELSE 0
    ...
    SELECT
        ...
        ot.document_number,
        ot.description,
        SUM(ot.signed_amount) OVER (ORDER BY ot.transaction_date, ot.id) as running_balance
...
BEGIN
    ...
    WITH customer_balances AS (
        ...
        WHERE ct.transaction_type IN ('Satış', 'Alış', 'Borç', 'Alacak')
...
-- Create RLS policies for customer_transactions
